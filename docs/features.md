...
- pinned **Trash** quick folder (type `trash`): `Enter` opens it in Finder, its preview shows the item count, and `Cmd+D` empties it via Finder (confirmed, since it's permanent). Typing `empty`, `trash`, or a close typo (`tarsh`) puts an **Empty Trash** row first in the results, selected, and `Enter` on it opens the same confirm prompt
- typing `font cache` (or `clear font cache`, `reset fonts`, or a close typo) puts a **Clear Font Cache** row first in the results, selected; `Enter` runs `atsutil databases -removeUser` right away (nothing confirmed - nothing is lost, the cache rebuilds itself after you log out or restart)
